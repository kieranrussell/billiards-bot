...
function getCommentReply(comment) {
  return new Promise(function(resolve, reject) {
    ...
    getData("http://api.snookerorg.ardalen.com/?t=10&s=2017")
      .then(function(response) {
        var playerId = response
          .filter(function(player) {
            return (
              keywords.indexOf(player.FirstName) !== -1 &&
              keywords.indexOf(player.LastName) !== -1
            );
          })
          .map(function(player) {
            returnData.name =
              ...
              " " +
              player.MiddleName.trim() +
              " " +
              player.LastName.trim();
            return player.ID;
          })[0];
        return getData(
          "http://api.snookerorg.ardalen.com/?t=8&p=" + playerId + "&s=2017"
        );
      })
      .then(function(response) {
      ...
      .then(function() {
        var reply =
          returnData.name.trim() +
          " is playing in the " +
          returnData.eventName.trim() +
          " on " +
          getDateString(returnData.date);
